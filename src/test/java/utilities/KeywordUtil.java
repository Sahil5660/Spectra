...
//		ExtentUtil.logger.get().log(Status.PASS, HTMLReportUtil.passStringGreenColor(logStep));
	...
	public static boolean writeInInput(String path, String type, String data, String logstep) {
		...
		WebElement element = GlobalUtil.getDriver().findElement(locatortype(type, path));
...
//		  URL url=new URL(qrCodeURL);
	...
	public static WebElement findWithFluintWait(final By locator, int seconds, int poolingMil) throws Exception {
		...
		try {
			Wait<WebDriver> wait = new FluentWait<WebDriver>(getDriver()).withTimeout(seconds, TimeUnit.SECONDS).pollingEvery(poolingMil, TimeUnit.MILLISECONDS)
	...
	public static WebElement findWithFluintWait(final By locator) throws Exception {
		...
		try {
			Wait<WebDriver> wait = new FluentWait<WebDriver>(getDriver()).withTimeout(DEFAULT_WAIT_SECONDS, TimeUnit.SECONDS).pollingEvery(200, TimeUnit.MILLISECONDS)
					.ignoring(NoSuchElementException.class).ignoring(StaleElementReferenceException.class).ignoring(ElementNotVisibleException.class);
...
//			System.out.println(sel.getOptions().toString());
	...
	public static void scrollAndClick(String visibleText) {
		mdriver.findElementByAndroidUIAutomator("new UiScrollable(new UiSelector().scrollable(true).instance(0)).scrollIntoView(new UiSelector().textContains(\""+visibleText+"\").instance(0))").click();
	}
	...
	public static void scroollonend(){
	...
	public static void scrollingtoparticularelement(String text){
		getDriver().findElement(MobileBy.AndroidUIAutomator("new UiScrollable(new UiSelector()).scrollIntoView(text("+text+"))"));
